...
var light1, light2;
var plyMesh, objMesh; // PLY and OBJ meshes
var material;
var PCAdata; 
var plane;
...
// Set up track bars and other GUI elements here...
    ...
    loader.load('model/mean_model_tri.ply', function (geometry) {
        ...
        if (geometry.isBufferGeometry) {
            ...
            for (let i = 0; i < positions.count; i++) {
                ...
                positions.setXYZ(i, geometryZero[i].x + diffx, geometryZero[i].y + diffy, geometryZero[i].z + diffz);
            }
        ...
        if (plyMesh) {
            scene.remove(plyMesh); // Remove existing mesh
        }

        plyMesh = new THREE.Mesh(geometry, material);
        plyMesh.position.set(0, 0, 0);
        plyMesh.rotation.set(-Math.PI / 2, 0, 0);
...
function init(data) {
    ...
    // Ground
    plane = new THREE.Mesh(new THREE.PlaneGeometry(40, 40), new THREE.MeshPhongMaterial({ ambient: 0x7799cc, color: 0x66aacc, specular: 0xdddddd }));
    ...
    // Material
    material = new THREE.MeshLambertMaterial({ color: 0xffffff, specular: 0xaaaaaa, shininess: 20, shading: THREE.SmoothShading });
    ...
    objLoader.load('_changes.obj', function (object) {
        if (objMesh) {
            scene.remove(objMesh); // Remove existing mesh
        }

        objMesh = object;
        objMesh.scale.set(0.08, 0.08, 0.08);
...
function animate() {
    ...
    // Update conditions for PLY and OBJ
    if (oldStt != anth.STATURE || oldBMI != anth.BMI || oldGen != anth.GENDER || oldAge != anth.AGE || oldSHS != anth.SHS || oldStd != anth.STUDY) {
        loadPLYFile(); // Update PLY
    }
...
// Additional functions like onDocumentMouseMove, calcCoords, etc.

// Ready to load data and animate
